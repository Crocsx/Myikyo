...
namespace HexasphereGrid
{
    ...
    public partial class Hexasphere : MonoBehaviour
    {
        ...
        public bool SetTileExtrudeAmount(int tileIndex, float extrudeAmount)
        {
            ...
            if (_style == STYLE.Invisible) return true;
        ...
        /// <summary>
        /// Changes all vertices elevation
        /// </summary>
        /// <param name="tileIndex">Tile index.</param>
        /// <param name="elevation">Elevation.</param>
        ...
        public List<int> GetTilesWithinSteps(int tileIndex, int maxSteps, bool usePathFinding = true)
        {
            ...
            else
            {
                ...
                for (int stepsFinished = 0; stepsFinished < maxSteps; stepsFinished++)
                {
                    ...
                    (resultsAtCurrentStep, resultsAtPreviousStep) = (resultsAtPreviousStep, resultsAtCurrentStep);
                }
            }
            return results;
        }
        ...
        public GameObject GenerateTileGameObject(int tileIndex, Material material = null, float extrusionAmount = -1)
        {
            ...
            return GenerateTileMesh(tileIndex, material, extrusionAmount);
        }
        ...
        public int GetTileAtUV(Vector2 uv)
        {
            ...
            return GetTileAtLocalPosition(localPos);
        }

        /// <summary>
        /// Gets the tile under a given position (in world or local space coordinates, see worldSpace parameter).
        /// </summary>
        ...
        /// <summary>
        /// Returns the tile under a given latitude/longitude
        /// </summary>
        ...
        public bool IsTileVisibleFromCamera(int tileIndex, Camera cam)
        {
            ...
            if (viewportPos.x < cam.rect.xMin || viewportPos.x >= cam.rect.xMax || viewportPos.y < cam.rect.yMin || viewportPos.y >= cam.rect.yMax)
                return false;

            // is it oriented to camera and not occluded by the sphere (ie. on the other side)?
            float posDistToCenter = (transform.position - pos).sqrMagnitude;
            Vector3 toCam = (cam.transform.position - pos).normalized;
            pos += toCam * 0.1f;
            float newPosDistToCenter = (transform.position - pos).sqrMagnitude;
            return newPosDistToCenter > posDistToCenter;
        }
